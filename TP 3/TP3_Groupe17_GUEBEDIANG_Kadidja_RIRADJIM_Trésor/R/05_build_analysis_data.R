...
# Construction de la base analytique W4 avec pondérations
# Architecture fidèle au repo GitHub
# =========================================================
...
if (is.na(path_indiv_w4) || is.na(path_health_w4) || is.na(path_cons_w4)) {
  stop("Un ou plusieurs chemins de fichiers indispensables sont introuvables.")
}

indiv_w4 <- haven::read_dta(path_indiv_w4) %>% janitor::clean_names()
health_w4 <- haven::read_dta(path_health_w4) %>% janitor::clean_names()
cons_w4 <- haven::read_dta(path_cons_w4) %>% janitor::clean_names()
...
to_chr_factor <- function(x) as.character(haven::as_factor(x))
...
make_quintile_safe <- function(x) {
  ...
  probs <- unique(probs)
  if (length(probs) < 6) return(rep(NA_character_, length(x)))
  as.character(cut(
    x,
    breaks = probs,
    include.lowest = TRUE,
...
indiv_core <- indiv_w4 %>%
  transmute(
    ...
    ea_indiv = if ("ea" %in% names(.)) to_num(ea) else NA_real_,
    zone_indiv = if ("zone" %in% names(.)) to_num(zone) else NA_real_
  )
...
health_core <- health_w4 %>%
  transmute(
    hhid = to_num(hhid),
    indiv = to_num(indiv),
    ea_health = if ("ea" %in% names(.)) to_num(ea) else NA_real_,
    zone_health = if ("zone" %in% names(.)) to_num(zone) else NA_real_,
    milieu_health = if ("sector" %in% names(.)) recode_sector_safe(sector) else NA_character_,
...
weight_var <- dplyr::case_when(
  "wt_wave4" %in% names(cons_w4) ~ "wt_wave4",
  "hhweight" %in% names(cons_w4) ~ "hhweight",
  TRUE ~ NA_character_
)

if (is.na(cons_var) || is.na(weight_var)) {
  stop("La base de consommation ne contient pas les variables attendues de consommation et de pondération.")
}
...
cons_core <- cons_w4 %>%
  transmute(
    ...
    poids_menage = to_num(.data[[weight_var]]),
    ea_cons = if ("ea" %in% names(.)) to_num(ea) else NA_real_,
    zone_cons = if ("zone" %in% names(.)) to_num(zone) else NA_real_,
    milieu_cons = if ("sector" %in% names(.)) recode_sector_safe(sector) else NA_character_
  ) %>%
  mutate(
...
analysis_w4 <- health_core %>%
  ...
  mutate(
    ea = dplyr::coalesce(ea_cons, ea_health, ea_indiv),
    zone = dplyr::coalesce(zone_cons, zone_health, zone_indiv),
    milieu = dplyr::coalesce(milieu, milieu_health, milieu_cons),
...
readr::write_csv(missing_summary, file.path(path_tables, "table_missing_summary.csv"))

analysis_counts <- tibble::tibble(
  analyse = c(
    "Base totale",
    "Morbidité (malade + sexe + âge_groupe + poids)",
    "Types de maladies",
  ...
  n_utilisable = c(
    ...
    sum(!is.na(analysis_w4$malade) & !is.na(analysis_w4$sexe) & !is.na(analysis_w4$age_groupe) &
          !is.na(analysis_w4$poids_menage) & analysis_w4$poids_menage > 0),
    sum((!is.na(analysis_w4$type_maladie_1) | !is.na(analysis_w4$type_maladie_2)) &
          !is.na(analysis_w4$poids_menage) & analysis_w4$poids_menage > 0),
    sum(!is.na(analysis_w4$prestataire_1) & !is.na(analysis_w4$poids_menage) & analysis_w4$poids_menage > 0),
    sum(!is.na(analysis_w4$dep_sante_totale) & analysis_w4$dep_sante_totale > 0 &
          !is.na(analysis_w4$poids_menage) & analysis_w4$poids_menage > 0),
    sum(!is.na(analysis_w4$dep_sante_totale) & analysis_w4$dep_sante_totale > 0 &
          !is.na(analysis_w4$milieu) & !is.na(analysis_w4$poids_menage) & analysis_w4$poids_menage > 0),
    sum(!is.na(analysis_w4$consulte) & !is.na(analysis_w4$quintile_conso) &
          !is.na(analysis_w4$poids_menage) & analysis_w4$poids_menage > 0)
  ),
  somme_poids = c(
    sum(analysis_w4$poids_menage, na.rm = TRUE),
    sum(analysis_w4$poids_menage[!is.na(analysis_w4$malade) & !is.na(analysis_w4$sexe) &
                                   !is.na(analysis_w4$age_groupe) & analysis_w4$poids_menage > 0], na.rm = TRUE),
    sum(analysis_w4$poids_menage[(!is.na(analysis_w4$type_maladie_1) | !is.na(analysis_w4$type_maladie_2)) &
                                   analysis_w4$poids_menage > 0], na.rm = TRUE),
    sum(analysis_w4$poids_menage[!is.na(analysis_w4$prestataire_1) &
                                   analysis_w4$poids_menage > 0], na.rm = TRUE),
    sum(analysis_w4$poids_menage[!is.na(analysis_w4$dep_sante_totale) & analysis_w4$dep_sante_totale > 0 &
                                   analysis_w4$poids_menage > 0], na.rm = TRUE),
    sum(analysis_w4$poids_menage[!is.na(analysis_w4$dep_sante_totale) & analysis_w4$dep_sante_totale > 0 &
                                   !is.na(analysis_w4$milieu) & analysis_w4$poids_menage > 0], na.rm = TRUE),
    sum(analysis_w4$poids_menage[!is.na(analysis_w4$consulte) & !is.na(analysis_w4$quintile_conso) &
                                   analysis_w4$poids_menage > 0], na.rm = TRUE)
  )
)

readr::write_csv(analysis_counts, file.path(path_tables, "table_effectifs_par_analyse.csv"))

cat("\nRésumé rapide de la base analytique :\n")
cat("N lignes :", nrow(analysis_w4), "\n")
cat("N colonnes :", ncol(analysis_w4), "\n")
cat("Poids manquants :", sum(is.na(analysis_w4$poids_menage)), "\n")
cat("Poids nuls ou négatifs :", sum(!is.na(analysis_w4$poids_menage) & analysis_w4$poids_menage <= 0), "\n")

if (all(is.na(analysis_w4$poids_menage))) {
  stop("La variable poids_menage est entièrement manquante.")
}
...
readr::write_csv(analysis_w4, file.path(path_clean, "analysis_w4.csv"))
