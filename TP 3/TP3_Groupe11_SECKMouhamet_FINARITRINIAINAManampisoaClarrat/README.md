...
Ce projet analyse les données de santé de l'Enquête Générale des Ménages du Nigeria (GHS) - vague 4 (2018-2019). Il étudie la morbidité, le recours aux soins, les dépenses de santé et leurs liens avec les caractéristiques démographiques et socio-économiques.
...
## Données utilisées
- `sect4a_harvestw4.dta` : Module santé
- `sect1_harvestw4.dta` : Composition des ménages
- `secta_harvestw4.dta` : Poids de sondage
- `totcons_final.dta` : Dépenses de consommation

## Analyses réalisées
- Statistiques descriptives sur la morbidité, le recours aux soins et les dépenses
- Estimations des taux de morbidité (survey)
...
2. Installer les dépendances : `install.packages(c("haven", "dplyr", "ggplot2", "naniar", "tidyr", "scales", "rstatix", "survey", "Hmisc"))`
