...
library(pacman)
p_load(haven, dplyr, ggplot2, naniar, tidyr, scales, rstatix, gtsummary, gt, forcats, binom, sf, viridis)
...
# 2.4. Tableau récapitulatif des niveaux d'éducation
...
# 2.5. Jointure avec sect1 (sexe, âge, zone, état)
...
# 3.1. Tableau des fréquences et proportions (base jointe)
...
# 3.2. Graphique : Barplot horizontal ordonné par fréquence
...
# 4.4. V de Cramér (intensité de l'association)
...
# 4.5. Graphique : Barres 100% empilées par sexe

props <- prop.table(tab_sexe_niveau, margin = 1)
...
# 5.6. Tableau récapitulatif : effectifs et médiane par groupe d'âge
...
# 6.2. Calcul des effectifs, proportions et IC (binomial exact)
...
# 6.3. Graphique : Barres côte à côte avec intervalles de confiance
...
# 7.4. Calcul du taux par État

taux_par_etat <- educ_demo %>%
  filter(!is.na(age), age >= 18, !is.na(sexe), !is.na(state)) %>%
  mutate(
...
# 7.5. Harmonisation des noms d'États (ajustements si nécessaire)
