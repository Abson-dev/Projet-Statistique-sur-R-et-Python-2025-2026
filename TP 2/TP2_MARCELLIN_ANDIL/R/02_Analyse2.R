...
# Jointure sect2 + sect1 sur hhid + indiv
...
educ_data <- educ_data %>%
  mutate(
    niveau_educ = case_when(
      i == 0                          ~ "Aucun",
      s2aq9 %in% c(1,2,3,51,52,61)       ~ "Aucun",       # pré-primaire/coranique → Aucun
...
ggplot(freq_educ,
  ...
  scale_x_continuous(
    limits = c(0, max(freq_educ$prop) * 1.2),
...
scol_data %>%
  ...
  mutate(
    prop    = n / sum(n),
    ic_low  = map2_dbl(n, sum(n), ~ binom.test(.x, .y)$conf.int[1]),
    ic_high = map2_dbl(n, sum(n), ~ binom.test(.x, .y)$conf.int[2])
  ) %>%
...
ggplot(heatmap_data,
  ...
  labs(
    title    = "Taux d'analphabétisme par État nigérian",
